...
from .common import DEFAULT_MAX_DEPTH, TARGET_OBJECT_NAME
...
async def find_specific_object(
    ...
    try:
        ...
        for child in children:
            result = await find_specific_object(child, target_name, level + 1, max_level)
...
async def get_child_objects(
    ...
    Оптимізовано: паралельна обробка дочірніх вузлів.
        ...
        async def process_child(child):
            try:
                ...
                if child_class == ua.NodeClass.Object:
                    ...
                    nested = await get_child_objects(child, level + 1, max_level)
...
async def find_node_by_path(
    ...
    try:
        ...
        # Якщо перша частина - це кореневий об'єкт, починаємо з нього
        if parts[0] == root_object_name:
            ...
            # Інакше шукаємо від кореневого об'єкта
            current_node = await find_object_by_name(client, root_object_name)
            if current_node is None:
                logger.error(f"Кореневий об'єкт '{root_object_name}' не знайдено")
                return None
